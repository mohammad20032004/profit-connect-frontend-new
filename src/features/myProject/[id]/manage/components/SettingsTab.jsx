        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Paper sx={{ p: 2.5, borderRadius: 3, height: '100%' }}>
            ...
            <Stack spacing={2}>
              ...
              <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: alpha(COLORS.navy, 0.05), border: '1px dashed', borderColor: 'divider' }}>
        ...
        <DialogContent>
          <Stack spacing={1}>
            ...
            {form.startDate && form.endDate && (
              <Typography variant="caption" color="text.secondary">
                {t('manage.dateRange', 'Project Period')}: {formatDate(form.startDate, lang)} — {formatDate(form.endDate, lang)}
              </Typography>
            )}
            <Typography variant="caption" color="text.secondary">
              {t('manage.status', 'Status')}: {form.status} • {t('manage.progress', 'Progress')}: {form.progress}%
            </Typography>
          </Stack>
        </DialogContent>
        <DialogActions>
