...
export function CardSkeleton({ hasCover = true }) {
  return (
    <Box sx={{ borderRadius: RADIUS, border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
...
export function StatsSkeleton() {
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, 1fr)', sm: 'repeat(4, 1fr)' }, gap: 2, mb: 3 }}>
      {[0, 1, 2, 3].map((i) => (
        <Box key={i} sx={{ p: 2, borderRadius: RADIUS, border: '1px solid', borderColor: 'divider' }}>
          <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1.5, alignItems: 'center' }}>
            <Skeleton variant="rounded" width={42} height={42} sx={{ borderRadius: RADIUS }} />
            <Box>
